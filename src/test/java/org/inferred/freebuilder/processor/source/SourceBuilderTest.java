...
public class SourceBuilderTest {
  ...
  public void testAddLine_trivialNestedTypeInDifferentPackage() {
    ...
    assertThat(code).contains("// Top-level should be imported: ImmutableList.Builder\n");
  }
  ...
  public void testAddLine_errorTypeArgument() {
    TypeElement myType =
        model.newType(
            "package com.example; class MyType {", "  java.util.List<NoSuchType<Foo>> foo;", "}");
    DeclaredType errorType =
        (DeclaredType) getOnlyElement(fieldsIn(myType.getEnclosedElements())).asType();
    ...
    source().addLine("package com.example;").addLine("%s", errorType);
  }
