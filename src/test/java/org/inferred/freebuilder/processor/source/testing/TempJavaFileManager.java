...
public class TempJavaFileManager extends ForwardingJavaFileManager<StandardJavaFileManager> {
  ...
  public Iterable<JavaFileObject> list(
    ...
    return () -> {
      Stream<JavaFileObject> inMemoryFiles =
          javaFiles.entrySet().stream()
  ...
  public JavaFileObject getJavaFileForOutput(
    ...
    checkArgument(
        ...
        "Unsupported location %s (must be one of %s)",
        location,
        LOCATIONS);
  ...
  private static String path(String packageName, String relativeName) {
    return packageName.replace('.', '/') + (packageName.isEmpty() ? "" : "/") + relativeName;
  }
