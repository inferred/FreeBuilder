...
class BuildableProperty extends PropertyCodeGenerator {
  ...
  static class Factory implements PropertyCodeGenerator.Factory {
    ...
    public Optional<BuildableProperty> create(Config config) {
      ...
      DeclaredType builder =
          maybeBuilder(datatype, config.getElements(), config.getTypes()).orElse(null);
      if (builder == null) {
        return Optional.empty();
      }

      BuildableType type =
          BuildableType.create(datatype, builder, config.getElements(), config.getTypes());
      ...
      return Optional.of(
          new BuildableProperty(config.getDatatype(), config.getProperty(), type, mutatorType));
    }
  }
  ...
  private BuildableProperty(
      Datatype datatype, Property property, BuildableType type, FunctionalType mutatorType) {
  ...
  private void addSetter(SourceBuilder code) {
    ...
    code.addLine(
        ...
        .addLine(
            "  if (%1$s == null || %1$s instanceof %2$s) {",
  ...
  private void addSetterTakingBuilder(SourceBuilder code) {
    code.addLine("")
        ...
        .addLine(
            ...
            datatype.getBuilder(), setter(property), type.builderType())
  ...
  private void addGetter(SourceBuilder code) {
    ...
    code.addLine("")
        .addLine("/**")
        .addLine(
            " * Returns a builder for the value that will be returned by %s.",
            datatype.getType().javadocNoArgMethodLink(property.getGetterName()))
        .addLine(" */")
  ...
  private void addFieldAssignment(
    ...
    if (type.suppressUnchecked() != Excerpts.EMPTY) {
      ...
      code.addLine(
          "  %s = (%s) %s;", finalField, property.getType(), property.getField().on(builder));
    }
    code.addLine("} else {")
        .addLine(
            "  %1$s %2$s %3$s = (%2$s) %4$s;",
  ...
  public void addAssignToBuilder(SourceBuilder code, Variable builder) {
    if (type.partialToBuilder() == PartialToBuilderMethod.TO_BUILDER_AND_MERGE) {
      code.add("%s = %s.toBuilder();", property.getField().on(builder), property.getField());
    } else {
      code.add(
          "%s = %s.mergeFrom(%s);",
